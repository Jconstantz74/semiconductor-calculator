...
function item_input_listener(e) {
	...
	const fabs = row.querySelectorAll(".US, .CH"); //capture the shop containers within this row
...
function calculate_totals() {
	...
	item_rows.forEach(function (row) {
		...
		const fabs = row.querySelectorAll(".US, .CH");
	 ...
	 */
	if (US < CH) { // if amazon's cost is less than both peapod and freshdirect, amazon is cheapest
		cheapest = 'U.S.';
	}

	if (CH < US) { // if freshdirect's cost is less than both peapod and amazon, freshdirect is cheapest
		cheapest = 'China';
	}
